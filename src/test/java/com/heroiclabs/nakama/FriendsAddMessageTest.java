...
public class FriendsAddMessageTest {
    ...
    public void friendsAddNotFound() throws Exception {
        FriendsAddMessage add = FriendsAddMessage.Builder.newBuilder().id(UUID.randomUUID().toString()).build();
        final Deferred<Boolean> deferred = client1.send(add);
    ...
    @Test
    public void friendsAddIdRequest() throws Exception {
        FriendsAddMessage add = FriendsAddMessage.Builder.newBuilder().id(session2.getId()).build();
        final Deferred<Boolean> deferred = client1.send(add);
    ...
    @Test
    public void friendsAddHandleRequest() throws Exception {
        FriendsAddMessage add = FriendsAddMessage.Builder.newBuilder().handle(session2.getHandle()).build();
        final Deferred<Boolean> deferred = client1.send(add);
        deferred.addErrback(new Callback<Error, Error>() {
            @Override
            public Error call(Error error) throws Exception {
    ...
    @Test
    public void friendsAddIdMutual() throws Exception {
    ...
    @Test
    public void friendsAddHandleMutual() throws Exception {
        FriendsAddMessage add1 = FriendsAddMessage.Builder.newBuilder().handle(session2.getHandle()).build();
        final Deferred<Boolean> deferred = client1.send(add1);
        deferred.addCallbackDeferring(new Callback<Deferred<Boolean>, Boolean>() {
            @Override
            public Deferred<Boolean> call(Boolean success) throws Exception {
                FriendsAddMessage add2 = FriendsAddMessage.Builder.newBuilder().handle(session1.getHandle()).build();
                return client2.send(add2);
            }
        }).addErrback(new Callback<Error, Error>() {
            @Override
